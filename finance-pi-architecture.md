...
> **구현 현황 (2026-07):** 이 문서는 최초 설계 문서로 보존한다. 12단계 로드맵은
> 전부 구현되었고(9단계 pre-2010은 스키마만), 이후 우선주 괴리·NPS 플로우
> 데이터셋, 팩터 13종과 크로스섹션 유틸, 한국형 거래비용 모델, 데이터 신뢰성
> 스코어카드, MCP 서버, 배포 스크립트가 추가되었다. 현재 기능 범위는
> `README.md`의 Current Scope, 정량 평가와 잔여 과제는
> `docs/project-review-2026-07-02.html`을 본다. 13장의 리스크 중 R6(코퍼레이트
> 액션)·R9(거래정지 플래그)·R10(캘린더)은 이후 실제 결함으로 확인되어 수정됐다.
